...
class SouthFourQuiz extends React.Component {
  ...
  onLoadSituation() {
    ...
    if (scores.length < 4) {
      this.setState({
        loadErrorMessage: new LocalizedMessage("allLast.error.few"),
      });
      return;
    }
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      if (isNaN(converted)) {
        this.setState({
          loadErrorMessage: new LocalizedMessage("allLast.error.NaN", {
            seat: `$t(${SEAT_NAMES[i]})`,
          }),
        });
        return;
  ...
  generateNewQuiz() {
    ...
    do {
      ...
      players[1].points - players[0].points < 1500 ||
      players[1].points - players[0].points > 16000
    );
  ...
  onSubmit(han, fu, tsumo, ronTarget, placementTarget, index, riichis) {
    ...
    let canBeEqual =
      (players[0].seat + 3) % 4 < (players[placementTarget].seat + 3) % 4;
    let dealer = players[0].seat === 0;

    if (tsumo) {
      required = findMinimumTsumoValue(
        players,
        scores,
        placementTarget,
        this.state.maxFu,
      ...
      if (
        (!dealer &&
          points[0] === required.nondealer &&
          points[1] === required.dealer) ||
        (dealer && points[0] === required.dealer)
      ) {
        message.feedback = "allLast.correct";
      } else if (
        (!dealer &&
          (points[0] > required.nondealer || points[1] > required.dealer)) ||
        (dealer && points[0] > required.dealer)
      ) {
        message.feedback = "allLast.tooMuch";
      }
    } else {
      required = findMinimumRonValue(
        players,
        scores,
        ronTarget,
        placementTarget,
      ...
      if (
        (!dealer && points === required.nondealer) ||
  ...
  render() {
    ...
    let scores = this.state.players.map((player, index) => {
      if (this.state.showDifferences) {
        return (
          <Row key={index}>
            {t(SEAT_NAMES[player.seat]) + ": " + player.points} (
            {index === 0
              ? t("allLast.you")
              : "+" + (player.points - this.state.players[0].points)}
            )
          </Row>
        );
      } else {
        ...
        <ListGroup>
            ...
            <Row>
              <Col xs="12" sm="8" md="6">
                <InputGroup>
                  ...
                  <Input
                    type="number"
                    value={this.state.maxFu}
                    placeholder={t("allLast.fu")}
                    step="5"
                    min="20"
                    max="130"
                    onBlur={this.onFuChanged}
            ...
            <Row>
              <Col className="form-check form-check-inline">
                <Input
                  ...
                  onChange={() => this.onToggleDifferences()}
                />
                <Label className="form-check-label" for="toggleDifference">
                  {t("allLast.showDifferences")}
                </Label>
              </Col>
            </Row>
          </ListGroupItem>
          <ListGroupItem>
            ...
            <InputGroup>
              <Input
                id="loadScoresString"
                placeholder="28000,26000,24000,22000"
              />
              <InputGroupAddon addonType="append">
                <Button color="warning" onClick={() => this.onLoadSituation()}>
                  {t("allLast.loadLabel")}
                </Button>
              </InputGroupAddon>
            </InputGroup>
            <Row>
              {this.state.loadErrorMessage
                ? this.state.loadErrorMessage.generateString(t)
                : ""}
            </Row>
          </ListGroupItem>
          <ListGroupItem>
            <Button onClick={() => this.generateNewQuiz()}>
              {t("allLast.newLabel")}
            </Button>
          </ListGroupItem>
          <ListGroupItemHeading>
            <span>{t("allLast.escapeHeader")}</span>
          </ListGroupItemHeading>
          <ListGroupItem>
          ...
          />
          <ListGroupItemHeading>
            <span>{t("allLast.riichiHeader")}</span>
          </ListGroupItemHeading>
          <ListGroupItem>
            <Row>{t("allLast.info")}</Row>
            {scores}
          ...
          />
          <ListGroupItemHeading>
            <span>{t("allLast.higherHeader")}</span>
          </ListGroupItemHeading>
          <ListGroupItem>
            <Row>{t("allLast.info")}</Row>
            {scores}
...
function findMinimumTsumoValue(
  players,
  scores,
  placementTarget,
  maxFu,
  ...
  for (let i = 0; i < TSUMO_SCORES.length; i++) {
    ...
    if (players[0].seat === 0) {
      ...
      newScores[1] -=
        players[1].seat === 0
          ? TSUMO_SCORES[i].dealer
          : TSUMO_SCORES[i].nondealer;
      newScores[2] -=
        players[2].seat === 0
          ? TSUMO_SCORES[i].dealer
          : TSUMO_SCORES[i].nondealer;
...
function findMinimumRonValue(
  ...
  for (let i = 0; i < RON_SCORES.length; i++) {
    ...
    let points =
      players[0].seat === 0 ? RON_SCORES[i].dealer : RON_SCORES[i].nondealer;
    newScores[0] += points;
