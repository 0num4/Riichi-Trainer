  ...
  translations: {
    ...
    trainer: {
      ...
      error: {
        ...
        wallEmptyShuffle:
          "山に残っている牌が足りないため、新しい手牌を作れません。混ぜ直します。",
      },
      playerNames: {
        you: "自家",
        right: "下家",
    ...
    history: {
      ...
      verbose: {
        ...
        loweredShanten:
          " シャンテン数が上がり、テンパイから遠ざかってしまいました。",
        optimal: " 最も牌効率の良い捨て牌は",
        optimalSpoiler: "{{tile}}で、",
        best: " 今の捨て牌は、最大効率でした!",
        exceptionalNoten:
          " テンパイ形になりましたが、あがり牌がすべて自分の手の中にあります。ほとんどのルールでは、この形はテンパイとして認められません。待ち受け方を変えましょう。",
        furiten:
          " フリテンになってしまいました。この手はロンあがりが出来ません。なぜなら、あがり牌を既に捨ててしまっているからです。",
        furitenWarning:
          " フリテンに要注意！テンパイに向かう上で有効な牌のいくつかが、既に捨て牌にあります。",
    ...
    allLast: {
      ...
      loadInstructions:
        "カンマ区切りで持ち点を入力してください。東,南,西,北の順で読み込みます。",
      loadLabel: "点棒状況を読み込む",
      ...
      higherHeader: "着順アップ",
      question:
        " {{action}} {{placement}} になるために、必要最低限のあがりは？",
      riichiQuestion:
        "もし {{player}} がリーチした場合、{{action}} {{placement}} になるために必要最低限のあがりは？",
      ron: " {{target}} からロンして ",
      tsumo: "ツモで",
      han: "飜",
      fu: "符",
      submit: "回答",
      wrong:
        "間違いです！ そのあがりでは {{placement}} になりません！ 範囲選択で正解を表示 → ",
      tooMuch:
        "そのあがりで {{placement}} に届きますが、必要最低限ではありません。 範囲選択で正解を表示 → ",
    ...
    analyzer: {
      ...
      instructions3:
        'Then, right click the link that appears and choose "Save As" or "Save Link As".',
      instructions4:
        'Finally, click "Browse..." and upload the file you saved.',
      instructions5:
        "Alternatively, if you have a mjlog file on your computer, you can rename it to end in .zip.",
      ...
      instructions8:
        "The disclaimer from the efficiency trainer applies here as well.",
      instructions9:
      ...
      noName: "Unknown",
      startingHand:
        "Your starting hand is {{hand}}. It's {{count}} tile from ready. The dora indicator is {{dora}}.",
      unknownAction:
        "Hey, this replay has something I don't account for. Please send me this replay so I can add it. {{debugInfo}}",
      ...
      chosenSafety:
        "The {{tile}}'s safety rating is {{rating}}. ({{- explanation}}) ",
      correctSafety: "That was the safest tile.",
      bestSafety:
        "Safest tile: {{tile}}, with a safety rating of {{rating}}. ({{- explanation}})",
      safetyExplanations: {
    ...
    defense: {
      ...
      averagedSafetyRating:
        "Against multiple riichis, the safety ratings will be averaged together, and the highest chosen as best.",
